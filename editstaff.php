...
if (isset($_POST["park"])) {
    ...
    $birth = $_POST['birth'];
    $upload = $_POST['upload'];
    $update=mysqli_query($con,"UPDATE staff SET surname='$surname',fname='$fname',mname='$mname',password='$password',email='$email',birth='$birth', upload='$upload' WHERE student_id='$id'");
            if ($update) {
                include 'change.php';
            }else{
                ...
                $id = $_GET['update'];
                $select = mysqli_query($con,"select * from sss where staff_id= '$id'");
                if ($select) {
